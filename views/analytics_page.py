        ...
        if uploaded_file:
            try:
                ...
                if numeric_cols:
                    ...
                    # AI Data Insights Generation
                    st.markdown("---")
                    st.markdown(f"### 🧠 {t('تحليل البيانات بالذكاء الاصطناعي', 'AI Data Intelligence')}")
                    if st.button(t("🚀 استخراج رؤى استراتيجية من هذه البيانات", "🚀 Extract Strategic Insights"), use_container_width=True, type="primary"):
                        with st.spinner(t("يقوم الذكاء الاصطناعي بقراءة المؤشرات وتحليل الاتجاهات...", "AI is reading metrics and analyzing trends...")):
                            from ai_engine.ai_gateway import structured_text_generation
                            
                            data_summary = df.describe().to_json()
                            samples = df.head(5).to_json()
                            
                            sys_prompt = f"""You are OMNI-ANALYST, a Senior Business Data Strategist. 
Analyze the provided statistical summary and data samples with extreme precision.

Objectives:
1. Identify hidden correlations and growth opportunities.
2. Spot anomalies or efficiency gaps (e.g. high spend but low conversion).
3. Provide 3 specific, actionable strategic recommendations based on the numbers.

Output a highly professional 3-paragraph executive summary in Arabic."""
                            
                            insights = structured_text_generation(sys_prompt, f"Data Summary: {data_summary}\n\nSamples: {samples}", preferred_provider="google")
                            
                            if insights:
                                st.session_state.data_insights = insights
                                
                    if st.session_state.get("data_insights"):
                        st.success(t("تم استخراج الرؤى الاستراتيجية بنجاح!", "Strategic insights extracted successfully!"))
                        st.markdown(f"""
