...
def render():
    ...
    with tab5:
        ...
        # Initialize real chat history
        if "chat_history" not in st.session_state:
            st.session_state.chat_history = []
            
        chat_input = st.text_input(t("اسألني عن استراتيجياتك وحملاتك...", "Ask me about your strategies and campaigns..."))
        
        if chat_input:
            from ai_engine.ai_gateway import structured_text_generation
            
            with st.spinner(t("المساعد الذكي يفكر...", "AI Assistant is thinking...")):
                sys_prompt = "You are OMNI-ELITE, a world-class marketing AI assistant by MTSE. You provide extremely strategic, professional, and actionable business advice. Speak confidently and answer the user directly in Arabic."
                ...
                response = structured_text_generation(sys_prompt, user_prompt, preferred_provider="openai")
                
                if response:
                    st.session_state.chat_history.append({"user": chat_input, "ai": response})
                else:
                    st.error(t("حدث خطأ في الاتصال بالذكاء الاصطناعي.", "Error connecting to AI Provider."))
                    st.session_state.chat_history.append({"user": chat_input, "ai": "عذرًا، فشل الاتصال بمحرك الذكاء الاصطناعي."})
        ...
        for msg in st.session_state.chat_history[-5:]:
            ...
            <div style="margin:8px 0;">
                ...
                <div class="glass-card" style="border-right:3px solid #06b6d4; padding:12px 18px; margin:4px 0;">
                    <strong>🤖 OMNI-ELITE:</strong> {msg['ai']}
                </div>
            </div>
            """, unsafe_allow_html=True)
